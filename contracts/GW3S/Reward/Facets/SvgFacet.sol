...
import {Svg, LibSvg} from "../../libraries/LibSvg.sol";

contract SvgFacet is Modifiers {

    event RewardSvg(Svg indexed svg, uint256 levelId, uint256 _type);
    /***********************************|
   |             Read Functions         |
   ...
   function deploySvg(string calldata _svg) external onlyOwner returns(address contracts) {
    ...
    ///@param _type the type of the level to query {LEVEL = 0, HIDDEN = 1, HACKER = 2}
    function storeSvg(string[] calldata _svg, uint256 levelId, uint256 _type) external onlyOwner returns(Svg memory contracts){
        SvgStorage storage s = LibAppStorage.svgDiamondStorage();
        require(_type <= 2, "storeSvg: type does not exist");
    ...
    ///@param levelId the level identifier of the token to query
    ///@param _type the type of the level to query {LEVEL = 0, HIDDEN = 1, HACKER = 2}
    function updateSvg(string[] calldata _svg, uint256 levelId, uint256 _type) external onlyOwner returns(Svg memory contracts) {
        require(_type <= 2, "storeSvg: type does not exist");
