...
contract Router {
    ...
    function removeLiquidityWithPermit(
        ...
        IUniswapV2Pair(pair).permit(msg.sender, address(this), value, block.timestamp, v, r, s);
        (amountA, amountB) = removeLiquidity(tokenA, tokenB, liquidity, amountAMin, amountBMin, to, block.timestamp, factory);
    }
    ...
    function _swap(uint[] memory amounts, address[] memory path, address _to,
        address factory) internal virtual {
    ...
    function _swapSupportingFeeOnTransferTokens(address[] memory path, address _to,
        ...
        for (uint i; i < path.length - 1; i++) {
            ...
            amountInput = IERC20Router(input).balanceOf(address(pair)).sub(reserveInput);
